...
export default async function Page({ searchParams }: { searchParams: SearchParams }) {
    ...
    return (
        <div className="mx-auto max-w-5xl p-8">
            ...
            <h1 className="mb-2 text-2xl font-semibold">Server 与 Client 组件示例</h1>
            ...
            <Suspense
                fallback={
                    <div className="mb-4 rounded border px-3 py-2 text-sm text-zinc-600">
                        统计加载中…
                    </div>
                }
            >
            ...
            <div className="mb-4 flex gap-2">
                <TabButton
                    href={`/server-client?tab=server&page=${initPage}&ps=${initPageSize}`}
                    active={initTab === 'server'}
                    color="indigo"
                >
                    只读列表（Server）
                </TabButton>
                <TabButton
                    href={`/server-client?tab=table&page=${initPage}&ps=${initPageSize}`}
                    active={initTab === 'table'}
                    color="sky"
                >
                    交互表格（Client）
                </TabButton>
                <TabButton
                    href={`/server-client?tab=form&page=${initPage}&ps=${initPageSize}`}
                    active={initTab === 'form'}
                    color="emerald"
                >
                    表单（Client）
