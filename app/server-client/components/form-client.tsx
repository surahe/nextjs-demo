...
export default function FormClient({ initial }: { initial: FormInitData }) {
    ...
    const [serverState, formAction, serverPending] = useActionState<ServerSubmitState, FormData>(serverSubmitAction, {});
    ...
    return (
        <div className="space-y-4">
            ...
            <form onSubmit={onSubmit} className="space-y-4">
                ...
                <button
                    disabled={pending}
                    className={`px-4 py-2 rounded text-white ${pending ? 'bg-zinc-400' : 'bg-emerald-600 hover:bg-emerald-700'}`}
                >
                    {pending ? '提交中…' : '提交'}
                </button>
            ...
            <div className="border-t pt-4">
                <div className="text-sm mb-2">通过 Server Action 提交</div>
                <form action={formAction} className="flex items-end gap-4">
                    <div>
                        <label className="block text-sm mb-1">用户名</label>
                        <input name="username" defaultValue={initial.username} className="rounded border px-3 py-2" />
                    </div>
                    <div>
                        <label className="block text-sm mb-1">性别</label>
                        <select name="gender" defaultValue={initial.gender} className="rounded border px-3 py-2">
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded text-white ${serverPending ? 'bg-zinc-400' : 'bg-sky-600 hover:bg-sky-700'}`}
                    >
                        {serverPending ? '提交中…' : '服务端提交'}
                    </button>
                </form>
                {serverState?.message ? <div className="mt-2 text-sky-700">{serverState.message}</div> : null}
            </div>
        </div>
    );
}
