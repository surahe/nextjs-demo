...
@theme {
  --color-brand: #3b82f6;
    ...
    },
    {
        ...
        detail: 'CSS 原生的级联层特性。Tailwind 默认分为 theme, base, components, utilities。后声明的层优先级高于前面的层。它本身不注册工具类，只是决定普通 CSS 的权重。',
...
@layer utilities {
  ...
  .text-balance { text-wrap: balance; }
    ...
    },
    {
        ...
        detail: 'v4 新增指令。它不仅仅是写一段 CSS，而是告诉 Tailwind：“这是一个工具类”。这意味着 Tailwind 会自动为它生成 hover:、focus:、md: 等变体，并支持按需打包。',
        example:
            '@utility btn { background-color: var(--color-primary); color: white; padding: 0.5rem 1rem; border-radius: var(--radius-md); }',
...
/* <span class="badge hover:badge-hover">（配合嵌套或其他工具类）</span> */`,
        practice:
            '当你需要一个自定义类，并且希望它能配合 hover: 或 lg: 一起使用时，必须用 @utility 而不是 @layer utilities。',
        preview: (
            <div className="flex items-center gap-4">
                <span className="bg-primary inline-flex cursor-pointer items-center rounded-full px-3 py-1 text-xs text-white transition-opacity hover:opacity-80">
                    基础 utility 效果
                </span>
                <span className="text-xs text-gray-500">
                    ← 假设这是 @utility btn，自动支持 hover 变体
                </span>
            </div>
        ),
    },
    {
        ...
        detail: '常用于提取重复的工具类组合。注意：它只是 CSS 的宏替换，不会生成新的工具类（除非你把它包在 @utility 里）。',
...
@utility btn-primary {
  @apply rounded-md bg-blue-600 px-4 py-2 font-medium text-white hover:bg-blue-700;
        ...
        practice:
            '不推荐滥用。如果只是想复用样式，React 组件本身是更好的复用边界。但对于全局通用的简单按钮样式，@apply 很方便。',
        preview: (
    ...
    },
    {
        ...
        detail: '用 @theme 定义的变量，Tailwind 会自动生成对应的工具类（如 bg-brand）。最重要的是，生成的工具类里会保留 CSS 变量引用（如 background-color: var(--color-brand)），这意味着你可以通过覆盖 CSS 变量来做主题切换。',
...
@layer base {
  ...
  .theme-dark {
    ...
    },
    {
        ...
        detail: '与 @theme 相对，inline 模式生成的工具类不会引用 CSS 变量，而是直接把十六进制颜色或具体数值内联进去。这样做生成的 CSS 更干净，但失去了动态覆盖变量的能力。',
        example:
            '@theme inline { --color-static-red: #ef4444; --font-mono: var(--font-geist-mono); }',
        fullExample: `/* 这里通常用于：
...
/* .bg-brand-fixed { background-color: #ff0000; } */`,
        practice:
            'Next.js 项目中，用于对接 next/font 注入的 --font-geist-sans 等变量，或者定义永远不变的纯黑色/纯白色。',
        preview: (
    ...
    },
    {
        ...
        detail: '允许你在 CSS 中为现有的变体（如 hover, focus, dark）编写自定义样式块。它将样式包裹在对应变体的逻辑中。',
  ...
  body {
    ...
    @variant dark {
      ...
      color: #ededed;
        ...
        practice:
            '常用于全局的基础样式设定，比如在 body 级别直接用 @variant dark 设置背景色，而不是在 HTML 上写一长串 dark:bg-black。',
        preview: (
    ...
    },
    {
        ...
        detail: '把复杂的 CSS 选择器映射成一个简短的 Tailwind 变体前缀。让你可以在 HTML 中像用 hover: 一样使用你自定义的条件。',
...
@custom-variant group-error (:merge(.group).error &);`,
        practice:
            "与 Radix UI 或其他无头组件库配合时，用来把 [data-state='open'] 映射为 open: 变体。",
        preview: (
            <div className="flex gap-2">
                <div className="flex items-center justify-center rounded border border-gray-200 bg-gray-100 px-3 py-1 text-xs text-gray-500 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-400">
                    默认状态
                </div>
    ...
    },
    {
        ...
        detail: 'v4 默认会自动扫描当前目录下的所有相关文件。但如果你的项目是 Monorepo，或者引用了外部包里的组件，就需要用 @source 手动把那些路径加进来。',
...
@source "./content/**/*.mdx";`,
    ...
    },
    {
        ...
        detail: 'v4 中插件不再写在 tailwind.config.js 里，而是直接在 CSS 中通过 @plugin 引入。支持官方插件和第三方插件。',
...
<article class="prose lg:prose-xl">
  <h1>...</h1>
        ...
        practice:
            '当你需要大段文本排版（typography）、原生表单元素重置（forms）或者动画增强时使用。',
        preview: (
            <div className="rounded-lg border border-black/5 bg-white p-4 dark:border-white/10 dark:bg-slate-900">
                <div className="space-y-2 border-l-2 border-gray-200 pl-4 dark:border-gray-700">
                    <h4 className="font-serif text-sm font-bold text-gray-900 dark:text-gray-100">
                        这是 Prose 插件效果模拟
                    </h4>
                    <p className="text-xs leading-relaxed text-gray-600 dark:text-gray-400">
                        插件自动为原生的{' '}
                        <code className="rounded bg-gray-100 px-1 py-0.5 text-[10px] text-red-500 dark:bg-gray-800">
                            h1
                        </code>
                        ,{' '}
                        <code className="rounded bg-gray-100 px-1 py-0.5 text-[10px] text-red-500 dark:bg-gray-800">
                ...
                <header className="flex flex-col gap-4 border-b border-slate-200 pb-8 dark:border-slate-800">
                    <div className="flex items-center gap-3">
                        <div className="flex size-10 items-center justify-center rounded-xl bg-sky-500 text-white shadow-sm">
                            ...
                            >
                                <path
                                    d="M12 4.5C12 4.5 10 9 5.5 9C10 9 12 13.5 12 13.5C12 13.5 14 9 18.5 9C14 9 12 4.5 12 4.5Z"
                                    fill="currentColor"
                                />
                                <path
                                    d="M19.5 15.5C19.5 15.5 18.5 17.5 16 17.5C18.5 17.5 19.5 19.5 19.5 19.5C19.5 19.5 20.5 17.5 23 17.5C20.5 17.5 19.5 15.5 19.5 15.5Z"
                                    fill="currentColor"
                                />
                                <path
                                    d="M5.5 16C5.5 16 4 19 0 19C4 19 5.5 22 5.5 22C5.5 22 7 19 11 19C7 19 5.5 16 5.5 16Z"
                                    fill="currentColor"
                                />
                            </svg>
                        </div>
                    ...
                    <p className="max-w-2xl text-sm leading-relaxed text-slate-600 dark:text-slate-400">
                        Tailwind CSS v4 采用了全新的“CSS 优先”配置模式，彻底废弃了
                        tailwind.config.js。现在，所有的配置、主题变量、插件加载和自定义工具类，全部通过
                        CSS 中的{' '}
                        <code className="rounded bg-slate-200 px-1.5 py-0.5 font-mono text-slate-800 dark:bg-slate-800 dark:text-slate-200">
                            @指令
                        </code>{' '}
                        来完成。本指南详细拆解了每个指令的原理与实战用法。
                    </p>
                </header>
                            ...
                            <div className="grid gap-6 md:grid-cols-2">
                                <div className="flex flex-col gap-4">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        ...
                                        <div className="rounded-lg border-l-4 border-sky-500 bg-sky-50 py-2.5 pr-4 pl-3 text-sm text-sky-800 dark:bg-sky-950/30 dark:text-sky-300">
                                            {item.practice}
                                        </div>
                                    </div>
                        ...
                        <div className="rounded-xl border border-slate-700 bg-slate-800 p-5">
                            ...
                            <ul className="flex flex-col gap-3 text-sm text-slate-300">
                                <li className="flex items-start gap-2">
                                    ...
                                    <div>
                                        <strong>@layer utilities</strong> 只是原生的 CSS
                                        层级控制。它
                                        <strong>不会</strong>注册 Tailwind 工具类，你
                                        <strong>不能</strong>在它身上使用{' '}
                                        <code className="text-sky-300">hover:</code> 等修饰符。
                                    </div>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="mt-0.5 text-emerald-400">✓</span>
                                    <div>
                                        <strong>@utility</strong> 是 Tailwind v4
                                        的魔法指令。它把你的 CSS 注册为一个
                                        <strong>真正的工具类</strong>
                                        ，自动支持响应式、暗色模式、状态伪类等所有 Tailwind 变体。
                                    </div>
                                </li>
                            </ul>
                        </div>
                        ...
                        <div className="rounded-xl border border-slate-700 bg-slate-800 p-5">
                            ...
                            <ul className="flex flex-col gap-3 text-sm text-slate-300">
                                <li className="flex items-start gap-2">
                                    ...
                                    <div>
                                        <strong>@theme</strong> 生成的类会保留{' '}
                                        <code className="text-sky-300">var(--xxx)</code>{' '}
                                        引用。这使得你可以在不同的 DOM
                                        节点或暗色模式下覆盖这个变量，实现主题切换。
                                    </div>
                                </li>
                                <li className="flex items-start gap-2">
                                    <span className="mt-0.5 text-slate-400">🔒</span>
                                    <div>
                                        <strong>@theme inline</strong> 生成的类会将值
                                        <strong>硬编码写死</strong>
                                        。适合永远不变的绝对常量，或者只是为了读取外部注入的变量（如
                                        next/font）。
                                    </div>
                                </li>
                            </ul>
